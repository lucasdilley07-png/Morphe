                ...
                if store.selectedCoachBuildSection == .builder {
                    ...
                    GlassCard {
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 10), count: 2), spacing: 10) {
                                CoachMenuField(title: "Fitness type") {
                                ...
                                CoachMenuField(title: "Target / focus") {
                    ...
                    GlassCard {
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            Text("When this draft feels right, save it to the archive so you can reopen it, assign it, or keep refining it later.")
                                .font(.subheadline)
                                .foregroundStyle(MorpheTheme.textSecondary)
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                case .athletes:
                    CoachFilteredNetworkFeed(
                        ...
                        posts: store.communityPosts.filter { $0.role == .client }
                    )
                case .coaches:
                    CoachFilteredNetworkFeed(
                        title: "Coaches",
                        subtitle: "Practical coaching notes, systems, and athlete wins from other coaches.",
                        posts: store.communityPosts.filter { $0.role == .coach }
                    )
                case .grow:
...
private struct CoachMessagesScreen: View {
    ...
    private var filteredThreads: [MessageThread] {
        let query = searchText.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        guard !query.isEmpty else { return store.messageThreads }
        return store.messageThreads.filter { thread in
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    if !filteredAthleteThreads.isEmpty {
                        coachThreadSection(title: "Athletes", threads: filteredAthleteThreads)
                    }

                    if !filteredCoachThreads.isEmpty {
                        coachThreadSection(title: "Coaches", threads: filteredCoachThreads)
                    }

                    if !filteredGroupThreads.isEmpty {
                        coachThreadSection(title: "Group Chats", threads: filteredGroupThreads)
                    }
                }
                .padding(16)
            }
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            ForEach(threads) { thread in
                Button {
                    ...
                    CoachInboxThreadRow(thread: thread)
                }
                .buttonStyle(.plain)
            }
        }
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack(spacing: 6) {
                    if thread.isGroupChat {
                        StatusBadge(text: "Group", color: MorpheTheme.warning)
                    }
                    Text(thread.preview)
                        .font(.caption)
                        .foregroundStyle(MorpheTheme.textSecondary)
