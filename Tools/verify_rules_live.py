...
def sign_up(tag: str) -> dict:
    ...
    response = requests.post(
        ...
        json={"email": email, "password": f"Rt-{RUN_ID}-9", "returnSecureToken": True},
        timeout=30)
...
for user, paths in [
    (A, [f"users/{A['uid']}/state/profile", f"users/{A['uid']}/following/{B['uid']}",
         f"users/{A['uid']}/blocked/{B['uid']}", f"usernames/{username}"]),
    (B, [f"posts/{post_id}/reactions/{B['uid']}"]),
]:
    for path in paths:
        fs_call(user, "DELETE", path)
for user in (A, B, C):
    requests.post(f"{AUTH}/accounts:delete?key={API_KEY}",
                  json={"idToken": user["token"]}, timeout=30)
print("  throwaway users deleted; report doc r-… stays (write-only by design —")
print("  resolve it via Tools/review_reports.py or the console).")
